...
El sistema se compila con Java 21 LTS y Spring Boot 3.2.x. La configuración principal se define en \texttt{application.properties} con valores por defecto para desarrollo y variables de entorno para producción (patrón 12-factor). Las dependencias principales se administran con Maven (\texttt{pom.xml}).
...
\begin{lstlisting}[language=Java, caption={Repositorio de conductores con consultas derivadas.}, label={lst:conductor-repo}]
@Repository
public interface ConductorRepository
    ...
    @Query("""
            ...
            """)
    Page<Conductor> buscarActivos(@Param("search") String search,
                                  Pageable pageable);
}
\end{lstlisting}

Los métodos \texttt{findByActivoTrue} y \texttt{buscar} utilizan consultas derivadas y JPQL estática respectivamente, sin concatenación de SQL dinámico.
...
\begin{lstlisting}[language=Java, caption={Declaración @NamedStoredProcedureQuery en Incidente.}, label={lst:incidente-sp}]
...
\begin{lstlisting}[language=Java, caption={Invocación del SP desde el repositorio.}, label={lst:incidente-repo}]
@Repository
public interface IncidenteRepository
        extends JpaRepository<Incidente, Long> {

    @Procedure(name = "Incidente.incidentesPorGravedad")
    List<IncidenteGravedadDTO> incidentesPorGravedad(
            @Param("p_tipo") String tipo);
}
\end{lstlisting}

El cursor REFCURSOR se lee dentro de la misma transacción JDBC, por lo que \texttt{ReporteService} está anotado con \texttt{@Transactional}.
...
\begin{lstlisting}[language=Java, caption={Invocación de función SQL escalar.}, label={lst:fn-programaciones}]
@Repository
public interface ProgramacionRepository
        extends JpaRepository<Programacion, Long> {

    @Query(value = "SELECT fn_total_programaciones(:estado)",
           nativeQuery = true)
    Long totalProgramaciones(@Param("estado") String estado);
}
\end{lstlisting}
