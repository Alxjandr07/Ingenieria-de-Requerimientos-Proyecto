...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{figuras/fig-ci-actions-3-verdes.jpeg}
  \caption{Pipeline CI (GitHub Actions) con tres ejecuciones consecutivas exitosas (criterio P1). Commits \texttt{4f4e6cd}, \texttt{17f484a}, \texttt{66fa254} en \texttt{main}.}
  \label{fig:ci-3-verdes}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{figuras/fig-render-deploy-succeeded-live.jpeg}
  \caption{Despliegue en Render con estado \texttt{Deploy succeeded | Live} (3m14s) y arranque verificado (\texttt{Tomcat started on port 8080 | Started SgroasApplication}).}
...
\begin{figure}[H]
  ...
  \caption{Servicio \texttt{sgroas-backend} en estado \texttt{Live} con URL pública \texttt{https://sgroas-backend.onrender.com} (Blueprint managed).}
...
OBS-09 & 3 / P3 Cobertura & 222 tests JUnit 5; JaCoCo núcleo 87,5\% instrucciones / 87,9\% ramas / 95,5\% líneas (\texttt{5f3b472}, reconciliado \texttt{17f484a}) \\
